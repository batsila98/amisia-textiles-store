...
class ControllerExtensionPaymentLiqPay extends Controller {
	public function index() {
		...
		// trofimenk0
		if($order_info['currency_code'] == 'UAN') {
			$currencyUAH = 'UAH';
		} else {
			$currencyUAH = $order_info['currency_code'];
		}
		...
		// trofimenk0
		if($order_info['prepayment']) {
			$this->load->model('extension/payment/prepayment');

			$prepayment_info = $this->model_extension_payment_prepayment->getPrepayment();

			if($prepayment_info) {
				$xml .= '	<amount>' . $this->currency->format($prepayment_info['value'], $order_info['currency_code'], $order_info['currency_value'], false) . '</amount>';
			} else {
		...
		$xml .= '	<currency>' . $currencyUAH . '</currency>';
