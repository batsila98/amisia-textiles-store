...
class ControllerSaleOrder extends Controller {
	...
	public function info() {
		...
		if ($order_info) {
			...
			$data['novaposhta_area'] = $order_info['novaposhta_area'];
			$data['novaposhta_city'] = $order_info['novaposhta_city'];
			$data['novaposhta_office'] = $order_info['novaposhta_office'];

			$data['ukrposhta_city'] = $order_info['ukrposhta_city'];
			$data['ukrposhta_office'] = $order_info['ukrposhta_office'];

			$data['prepayment'] = $order_info['prepayment'];

			if($data['prepayment']) {
				$this->load->model('extension/payment/prepayment');

				$prepayment_info = $this->model_extension_payment_prepayment->getPrepayment();
	
				if($prepayment_info) {
					$data['prepayment_value'] = $prepayment_info['value'];
					$data['prepayment_value_formatted'] = $this->currency->format($data['prepayment_value'], $order_info['currency_code']);
				} else {
				...
				foreach ($totals as $total) {
					if($total['code'] == 'total') {
						$data['total_order_value'] = $total['value'];
						$data['total_order_value_formatted'] = $this->currency->format($data['total_order_value'], $order_info['currency_code']);
					} else {
						$data['total_order_value'] = 0;
					}
				}

				$data['price_after_prepayment'] = $data['total_order_value'] - $data['prepayment_value'];
				$data['price_after_prepayment_formatted'] = $this->currency->format($data['price_after_prepayment'], $order_info['currency_code']);
